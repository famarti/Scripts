# -*- coding: utf-8 -*-
# Script para procesamiento y visualización de datos relacionados con mi tesis de licenciatura.
# https://www.youtube.com/watch?v=YsFC5ulG-zQ&t=4s&pp=ygUWdGVzaXMgZmFjdW5kbyBtYXJ0aW5leg%3D%3D
...
# Definición de fases y etiquetas
...
labels = [
    r"$NEE\ (ton\ de\ C/ha\ día)$",
    r"$GPP\ (ton\ de\ C/ha\ día)$",
    r"$Reco\ (ton\ de\ C/ha\ día)$",
    r"$Calor\ Sensible\ (W/m^2\ día)$",
    r"$Calor\ Latente\ (W/m^2\ día)$",
]
...
for var in Vars:
    ...
    if var == "Calor Sensible(W/m2)":
        plt.savefig(
            "sensible.png", format="png", dpi=1000, bbox_inches="tight", pad_inches=0.5
        )
    else:
        plt.savefig(
            str(var[:3]) + ".png",
...
# Eliminación de días con pocos datos
...
datos_rad = datos_rad.set_index(
    pd.date_range(start="12/21/2012", end="04/08/2013", freq="D").strftime("%Y-%m-%d")
)
...
ax2 = ax.twinx()
ax.plot(
    datos_rad.index, datos_rad.iloc[:, 0], color="red", linestyle="--", linewidth=2.5
)
...
plt.gcf().autofmt_xdate()
plt.savefig(
    "radiacion.png", format="png", dpi=1000, bbox_inches="tight", pad_inches=0.5
)
...
# Gráfico de barras de acumulados por fase
X = np.arange(6)
plt.figure(figsize=(15, 10))
...
plt.savefig(
    "acu_fases.png", format="png", dpi=1000, bbox_inches="tight", pad_inches=0.5
)

# Usando datos y datos_rad
# Estimación cada 10 días de QE/Rn y QH/RN
...
flux_vs_rad = pd.DataFrame(
    {
        "QH/RN": [
            (x / y) * 100
            for x, y in zip(Qx_10["Calor Sensible(W/m2)"], rad_10["rad"])
            if y != 0
        ],
        "QE/RN": [
            (x / y) * 100
            for x, y in zip(Qx_10["Calor Latente(W/m2)"], rad_10["rad"])
            if y != 0
        ],
    }
)
...
ax.bar(
    Y - width / 2, flux_vs_rad["QE/RN"], color="g", width=width, label=r"$Q_E/RN\ $ (%)"
)
ax.bar(
    Y + width / 2, flux_vs_rad["QH/RN"], color="b", width=width, label=r"$Q_H/RN\ $ (%)"
)
ax.set_xticks(Y)
...
fig.tight_layout()
plt.savefig(
    "flujos_vs_rad.png", format="png", dpi=1000, bbox_inches="tight", pad_inches=0.5
)
