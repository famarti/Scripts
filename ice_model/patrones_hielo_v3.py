...
#df = pd.read_csv('hielo2016360hs', header = None, sep = r'\s{1,}') 
...
#%%
#F_rot, T_matrix = _varimax(F_matrix)

#Z_rot = np.dot(Z_matrix, T_matrix) # considerar las primeras 8 (Richman, 1986)

#F_rot = np.dot(F_matrix, T_matrix) # rotated loadings

#variance = np.sum(F_rot ** 2, axis=0)
#F_rot = F_rot[:, list(reversed(np.argsort(variance)))].copy() # ordeno loadings por varianza

#load = np.concatenate((F_rot[:,0:8], F_matrix[:,8:]), axis=1)

#scores = np.concatenate((Z_rot[:,0:8], Z_matrix[:,8:]), axis=1) # Ahi van las 8
...
# #Tomamos las CP por ejemplo...
# nro_cp = 7
# data_test = full_prueba.iloc[:,2:nro_cp+2]
# latitu = df_latlon.iloc[:,0]
# longi = df_latlon.iloc[:,1]
...
#%%  # Autovalores asociados a cada CP. 
# avals = [ (np.sum(df_load.iloc[:,col].values))**2 for col in range(507) ]
# max_avals = sorted(avals, reverse = True)[:8] #los 8 mayores autovalores
# cp_max_var = [avals.index(ind) for ind in max_avals] #index de las 7 cp que mayor varianza explican
...
#col_names = [i for i in list(dti)]

#cols_d = {k:v for k,v in zip([i for i in range(508)],col_names)}
...
#grupo_1_pos = (listado.loc[(listado[0] >= 0.3) & (listado[0] >= 0), 0]).index
#grupo_1_neg = (listado.loc[(listado[0] <= -0.3) & (listado[0] <= 0), 0]).index

df_latlon.rename(columns = {0:'lat', 1:'lon'}, inplace = True)
data = pd.concat([df_latlon, df], axis = 1)
#data.rename(columns = cols_d, inplace = True)
