...
ref = np.load(
    "referencias.npy", allow_pickle=True
)  # archivo referencias.npy traido de la ejecucion de shapefile.py
iceberg_names = [
    str("A" + str(i)) for i in range(0, len(ref))
]  # 3220 nro de centroides = len(ref)
...
geo_24 = gpd.GeoDataFrame(
    ronda_24, geometry=gpd.points_from_xy(ronda_24.Longitude, ronda_24.Latitude)
)
...
geo_48 = gpd.GeoDataFrame(
    ronda_48, geometry=gpd.points_from_xy(ronda_48.Longitude, ronda_48.Latitude)
)
...
geo_72 = gpd.GeoDataFrame(
    ronda_72, geometry=gpd.points_from_xy(ronda_72.Longitude, ronda_72.Latitude)
)
...
class Tempano(object):
    ...
    def cercanos(self, other):
        if (
            abs(self.latitude - other.latitude) < 0.1
            and abs(self.longitude - other.longitude) < 0.1
        ):
            return True
        else:
            return False
        ...
        for tpano in ice_24:
            if tpano.posicion().within(geo_grilla["geometry"][grd_i]):
                if geo_grilla["colores"][grd_i] is 1:
                    ...
                    geo_grilla["colores"][grd_i] = redef_color(
                        geo_grilla["colores"][grd_i], tpano.color
                    )
                    ice_24.pop(ice_24.index(tpano))
            else:
